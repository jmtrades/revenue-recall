  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-[320px_1fr]">
      <div className="space-y-3">
        {canAuthor && !form && (
          <button onClick={() => { setForm(EMPTY_FORM); setError(null); }} className="w-full rounded-lg bg-brand px-3 py-2 text-sm font-medium text-white transition hover:bg-brand/90">
            New template
          </button>
        )}
        ...
        {form ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={save} disabled={busy || !form.name.trim() || !form.body.trim()} className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition hover:bg-brand/90 disabled:opacity-50">
