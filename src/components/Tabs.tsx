...
export function Tabs({ tabs, initial }: { tabs: { id: string; label: string; content: ReactNode }[]; initial?: string }) {
  ...
  return (
    <div>
      <div role="tablist" aria-label="Sections" className="mb-5 flex gap-1 overflow-x-auto border-b border-border [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {tabs.map((t) => (
          <button
            key={t.id}
            role="tab"
      ...
      <div role="tabpanel" id={`tabpanel-${active}`} aria-labelledby={`tab-${active}`}>{tabs.find((t) => t.id === active)?.content}</div>
    </div>
  );
}
