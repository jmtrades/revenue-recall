...
export function EmbeddedCheckoutModal({ request, onClose }: { request: CheckoutRequest | null; onClose: () => void }) {
  ...
  // A11y: this modal sits behind every billing surface — trap focus inside it,
  // close on Escape, and restore focus to the trigger on close (WCAG 2.1.2 /
  // 2.4.3), matching every other modal in the app.
  useEscapeKey(Boolean(request), onClose);
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Checkout" className="relative max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-border bg-surface p-4 shadow-[0_24px_80px_-20px_rgba(0,0,0,0.7)]" onClick={(e) => e.stopPropagation()}>
