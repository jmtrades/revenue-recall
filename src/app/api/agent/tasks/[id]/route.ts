...
export async function PATCH(req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    await updateTask((await params).id, { enabled: parsed.data.enabled });
    await recordAudit(parsed.data.enabled ? "agent.task_resumed" : "agent.task_paused", (await params).id);
...
export async function DELETE(_req: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await recordAudit("agent.task_deleted", (await params).id);
    return NextResponse.json({ ok: true });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : "Failed" }, { status: 500 });
  }
