...
export interface VoiceCall {
  ...
  /** ISO 639-1 language the call should be conducted in (STT + the agent's
   *  replies). Only sent for languages the live-call stack supports; absent
   *  means English. */
  lang?: string;
}
...
const webhookVoice: VoiceTransport = {
  ...
  async place({ to, from, context, voiceId, opener, voicemail, meta, lang }) {
    try {
      // Undefined fields are dropped by JSON.stringify, so this stays compatible
      // with any gateway; our in-house call-gateway uses context/voiceId/opener/voicemail/meta/lang.
      const r = await postWebhook(env("VOICE_WEBHOOK_URL")!, { channel: "voice", to, from: from ?? env("OUTBOUND_FROM_NUMBER"), context, voiceId, opener, voicemail, meta, lang });
 ...
 *  simply ignore it. */
export async function placeCall(to: string, opts: { from?: string; context?: string; voiceId?: string; opener?: string; voicemail?: string; meta?: Record<string, string>; lang?: string } = {}): Promise<SendResult> {
