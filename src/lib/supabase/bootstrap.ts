...
export async function bootstrapOrg(opts?: {
  ...
  // Provisioning is NOT a single DB transaction. If a later step fails after the
  // org row commits, the org would be orphaned — no member links the user to it,
  // so they're locked out AND every retry creates yet another dead org. Roll the
  // org back on any failure (FKs cascade to pipeline/stages/members/demo rows) so
  // the next sign-in retries cleanly. This also tidies up the race-loser's org
  // when the unique index on members.auth_user_id rejects a concurrent duplicate.
  try {
    // 2. Pipeline + stages
    ...
    // Best-effort compensating delete; the cascade removes everything created above.
    await client.from("orgs").delete().eq("id", orgId).then(() => {}, () => {});
    throw e;
  }
}
