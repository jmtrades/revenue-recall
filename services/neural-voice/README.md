...
| `elevenlabs` | ElevenLabs (hosted) | **Use ElevenLabs on live phone calls too.** The call gateway streams PCM from this service, so `VOICE_ENGINE=elevenlabs` makes calls speak in ElevenLabs with no gateway change. The one engine that calls a vendor (audio leaves your box). Needs `ELEVENLABS_API_KEY`; uses `eleven_turbo_v2_5` (quality-first real-time model — set `ELEVENLABS_MODEL=eleven_flash_v2_5` for lowest latency) and streams `pcm_24000` (Creator+ tier — set `ELEVENLABS_PCM_FORMAT=pcm_16000` otherwise). Delivery is tunable via `ELEVENLABS_STABILITY` / `ELEVENLABS_SIMILARITY` / `ELEVENLABS_STYLE`. Voice via `ELEVENLABS_VOICE_ID` / per-call `voiceId`. |
